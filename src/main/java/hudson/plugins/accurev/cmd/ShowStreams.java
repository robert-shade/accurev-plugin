...
public class ShowStreams extends Command {
  ...
  public static Map<String, AccurevStream> getStreams( //
    ...
    if (scm.isIgnoreStreamParent()) {
      ...
      streams =
          getAllAncestorStreams(
              nameOfStreamRequired, scm, accurevEnv, workspace, listener, launcher);
    } else {
      streams =
          getAllStreams(
              scm,
  ...
  private static Map<String, AccurevStream> getAllAncestorStreams( //
    ...
    while (streamName != null && !streamName.isEmpty()) {
      final Map<String, AccurevStream> oneStream =
          getOneStream(streamName, accurevEnv, workspace, listener, launcher, scm);
  ...
  private static Map<String, AccurevStream> getOneStream( //
    ...
    return AccurevLauncher.runCommand(
        "Restricted show streams command",
        scm.getAccurevTool(),
        launcher,
        cmd,
        scm.getOptionalLock(),
